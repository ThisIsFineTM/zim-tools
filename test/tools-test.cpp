#include <zim-tools/tools.h>  // for computeRelativePath, UriKind, decodeHtm...
...
#include "gtest/gtest.h"  // for Message, TestPartResult, CmpHelperEQ
...
TEST(CommonTools, computeAbsolutePath)
{
  ...
  str = computeAbsolutePath("/home/alex/oz/zim-tools/test/data/",
                            "minimal-content/hello.html");
  EXPECT_EQ(str,
            "/home/alex/oz/zim-tools/test/data/minimal-content/hello.html");
  ...
  // TODO: validate that this is really intended behavior / if there are any
  // places which rely on this behavior.
  str = computeAbsolutePath("/home/alex/oz/zim-tools/test/data",
                            "minimal-content/hello.html");
...
TEST(tools, uriKind)
{
  ...
  EXPECT_EQ(UriKind::SIP,
            uriKind("sip:1-999-123-4567@voip-provider.example.net"));
  EXPECT_EQ(UriKind::SIP,
            uriKind("SIP:1-999-123-4567@voip-provider.example.net"));
  ...
  EXPECT_EQ(UriKind::OTHER,
            uriKind("/xyz/javascript:console.log('hello, world!')"));

  EXPECT_EQ(UriKind::OTHER, uriKind("/"));
  EXPECT_EQ(UriKind::OTHER,
            uriKind("/api/data:text/plain;charset=UTF-8,qwerty"));
...
TEST(tools, isOutofBounds)
{
  ...
  ASSERT_TRUE(isOutofBounds(
      "../../../-/s/css_modules/ext.cite.ux-enhancements.css", "A/Blood_/"));
}
...
<html>
  ...
  // link
  EXPECT_LINKS("abcd href = qwerty src={123} xyz", "");
}
#undef EXPECT_LINKS
...
TEST(tools, httpRedirectHtml)
{
  ...
  EXPECT_EQ(httpRedirectHtml((const char*)u8"A/Κίουι"),
            "<!DOCTYPE html>"
            "<html>"
            "<head>"
            ...
            "content=\"0;url=A/%CE%9A%CE%AF%CE%BF%CF%85%CE%B9\" />"
            "</head>"
            "<body></body>"
            "</html>");
}
