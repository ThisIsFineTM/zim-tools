...
#include <zim-tools/tools.h>    // for CopyItem
...
#include <zim/writer/item.h>     // for Item
//
...
void create(const std::string& start_filename,
            const std::string& diff_filename,
            const std::string& out_filename)
{
  ...
  if (diff_archive.hasEntryByPath(mainPath)
      || start_archive.hasEntryByPath(mainPath)) {
  ...
  for (unsigned int index = 0; index < start_archive.getEntryCount(); index++) {
    ...
    try {
      ...
      zimCreator.addRedirection(
          entry.getPath(), entry.getTitle(), redirectPath);
      continue;
    } catch (...) {
      if (entry.isRedirect()) {
  ...
  for (unsigned int index = 0; index < +diff_archive.getEntryCount(); index++) {
    ...
    if (start_archive.hasEntryByPath(entry.getPath())
        || isAdditionalMetadata(entry.getPath())) {
      continue;
    }
...
void usage()
{
  std::cout << "\nzimpatch computes the end_file using a start_file and a "
               "diff_file (made by zimdiff).\n"
...
bool checkDiffFile(std::string startFileName, std::string diffFileName)
{
  ...
  for (unsigned int i = 0; i < additionalMetadata.size(); i++) {
    if (!diffArchive.hasEntryByPath(
            additionalMetadata[i]))  // If the article was not found in the
                                     // file.
      return false;
  }
  ...
  std::string startFileUID2
      = diffArchive.getEntryByPath(additionalMetadata[1]).getItem().getData();
  std::string temp = "";
  unsigned int k = 0;
  char tempArray[16];
...
int main(int argc, char* argv[])
{
  ...
  try {
    ...
    if (!checkDiffFile(start_filename, diff_filename)) {
      std::cout << "\n[ERROR]: The diff file provided does not match the given "
                   "start file.";
      return 0;
    }
