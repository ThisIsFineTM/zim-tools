...
#include <iostream>   // for basic_ostream, operator<<, basic_ost...
...
void create(const std::string& filename_1,
            ...
            const std::string& outpath)
{
  ...
  // Add All articles in file_2 that are not in file_1 and those that are not
  // the same in file_1
  ...
  for (auto& entry2 : archive_2.iterByPath()) {
    try {
      ...
      if (std::string(entry2.getItem().getData())
          != std::string(entry1.getItem().getData())) {
        auto tmpItem = std::shared_ptr<zim::writer::Item>(
            new CopyItem(entry2.getItem()));
        zimCreator.addItem(tmpItem);
        continue;
      }
    } catch (...) {  // If the article is not present in FILe 1
      if (entry2.isRedirect()) {
        zimCreator.addRedirection(entry2.getPath(),
                                  entry2.getTitle(),
                                  entry2.getRedirectEntry().getPath());
      } else {
        auto tmpItem = std::shared_ptr<zim::writer::Item>(
            new CopyItem(entry2.getItem()));
        zimCreator.addItem(tmpItem);
...
void usage()
{
  std::cout << "\nzimdiff computes a diff_file between two ZIM files, in order "
               "to facilitate incremental updates.\n"
