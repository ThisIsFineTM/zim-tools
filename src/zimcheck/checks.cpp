...
//
#include <zim-tools/concurrent_cache.h>     // for ConcurrentCache
#include <zim-tools/metadata.h>             // for Metadata
#include <zim-tools/progress.h>             // for ProgressBar
#include <zim-tools/tools.h>                // for html_link, adler32, gener...
#include <zim-tools/zimcheck/json_tools.h>  // for OutputStream, property
//
#include <zim/archive.h>  // for Archive, validate, Integr...
#include <zim/blob.h>     // for Blob
#include <zim/entry.h>    // for Entry
#include <zim/item.h>     // for Item
#include <zim/zim.h>      // for entry_index_type, cluster...
//
...
#include <mutex>               // for mutex, unique_lock, lock_...
...
std::unordered_map<LogTag, std::string> tagToStr
    = {{LogTag::ERROR, "ERROR"}, {LogTag::WARNING, "WARNING"}};
...
std::unordered_map<MsgId, MsgInfo> msgTable = {
    {MsgId::CHECKSUM,
     {TestType::CHECKSUM,
      "ZIM Archive Checksum in archive: {{&archive_checksum}}\n"}},
    {MsgId::MAIN_PAGE,
     {TestType::MAIN_PAGE,
      "Main Page Index stored in Archive Header: {{&main_page_index}}"}},
    {MsgId::EMPTY_ENTRY, {TestType::EMPTY, "Entry {{&path}} is empty"}},
    {MsgId::OUTOFBOUNDS_LINK,
     {TestType::URL_INTERNAL,
      "{{&link}} is out of bounds. Article: {{&path}}"}},
    {MsgId::DANGLING_LINKS,
     {TestType::URL_INTERNAL,
      "The following links:\n{{#links}}- "
      "{{&value}}\n{{/links}}({{&normalized_link}}) were not found in article "
      "{{&path}}"}},
    {MsgId::EXTERNAL_LINK,
     {TestType::URL_EXTERNAL,
      "{{&link}} is an external dependence in article {{&path}}"}},
    {MsgId::EMPTY_LINKS,
     {TestType::URL_EMPTY,
      "Found {{&count}} empty links in article: {{&path}}"}},
    {MsgId::REDUNDANT_ITEMS,
     {TestType::REDUNDANT, "{{&path1}} and {{&path2}}"}},
    {MsgId::METADATA, {TestType::METADATA, "{{&error}}"}},
    {MsgId::REDIRECT_LOOP,
     {TestType::REDIRECT, "Redirect loop exists from entry {{&entry_path}}\n"}},
...
{
  switch (tt) {
    case TestType::CHECKSUM:
      return "checksum";
    case TestType::INTEGRITY:
      return "integrity";
    case TestType::EMPTY:
      return "empty";
    case TestType::METADATA:
      return "metadata";
    case TestType::FAVICON:
      return "favicon";
    case TestType::MAIN_PAGE:
      return "main_page";
    case TestType::REDUNDANT:
      return "redundant";
    case TestType::URL_INTERNAL:
      return "url_internal";
    case TestType::URL_EXTERNAL:
      return "url_external";
    case TestType::URL_EMPTY:
      return "url_empty";
    case TestType::REDIRECT:
      return "redirect";
    default:
      throw std::logic_error("Invalid TestType");
  };
}
...
{
  return i1.getClusterIndex() == i2.getClusterIndex()
         && i1.getBlobIndex() == i2.getBlobIndex();
}
...
void ErrorLogger::jsonOutput(const MsgIdWithParams& msg) const
{
  ...
  jsonOutputStream << JSON::property("level",
                                     tagToStr[errormapping[m.check].first]);
  ...
  if (jsonOutputStream.enabled()) {
    ...
    for (size_t i = 0; i < size_t(TestType::COUNT); ++i) {
      ...
      if (!testStatus[i]) {
        ...
        std::cout << "[" + tagToStr[p.first] + "] " << p.second << ":"
                  << std::endl;
...
void test_checksum(zim::Archive& archive, ErrorLogger& reporter)
{
  ...
  if (!result) {
    ...
    reporter.addMsg(MsgId::CHECKSUM,
                    {{"archive_checksum", archive.getChecksum()}});
  }
}
...
void test_mainpage(const zim::Archive& archive, ErrorLogger& reporter)
{
  ...
  if (!testok) {
    reporter.addMsg(MsgId::MAIN_PAGE,
                    {{"main_page_index", toStr(archive.getMainEntryIndex())}});
  }
}
...
class ArticleChecker
{
 ...
 private:  // functions
  ...
  void check_internal_links(zim::Item item,
                            const GroupedLinkCollection& groupedLinks);
  void check_external_links(zim::Item item, const LinkCollection& links);

  bool is_valid_internal_link(const std::string& link)
  {
    return linkStatusCache.getOrPut(
        link, [this, link]() { return archive.hasEntryByPath(link); });
  }
...
void ArticleChecker::check_internal_links(zim::Item item,
                                          const LinkCollection& links)
{
  ...
  for (const auto& l : links) {
    ...
    if (l.link().front() == '#' || l.link().front() == '?')
      continue;
    if (l.isInternalUrl() == false)
      continue;

    if (isOutofBounds(l.link(), baseUrl)) {
      reporter.addMsg(MsgId::OUTOFBOUNDS_LINK,
                      {{"link", std::string{l.link()}}, {"path", path}});
      continue;
    }

    auto normalized = normalize_link(l.link(), baseUrl);
    groupedLinks[normalized].push_back(std::string{l.link()});
  }

  if (nremptylinks) {
    reporter.addMsg(MsgId::EMPTY_LINKS,
                    {{"count", toStr(nremptylinks)}, {"path", path}});
  }

  check_internal_links(item, groupedLinks);
}

void ArticleChecker::check_internal_links(
    zim::Item item, const GroupedLinkCollection& groupedLinks)
{
  const auto path = item.getPath();
  for (const auto& p : groupedLinks) {
    ...
    if (!is_valid_internal_link(link)) {
      ...
      reporter.addMsg(
          MsgId::DANGLING_LINKS,
          {{"path", path}, {"normalized_link", link}, {"links", links}});
      break;
    }
  }
}

void ArticleChecker::check_external_links(zim::Item item,
                                          const LinkCollection& links)
{
  const auto path = item.getPath();
  for (const auto& l : links) {
    if (l.attribute() == "src" && l.isExternalUrl()) {
      reporter.addMsg(MsgId::EXTERNAL_LINK,
                      {{"link", std::string{l.link()}}, {"path", path}});
      break;
    }
  }
    ...
    while (!l.empty()) {
      ...
      if (!l.empty()) {
        ...
        for (auto other : l) {
          ...
          reporter.addMsg(MsgId::REDUNDANT_ITEMS,
                          {{"path1", e1.getPath()}, {"path2", e2.getPath()}});
        }
...
class TaskStream
{
 ...
 public:  // functions
  explicit TaskStream(ArticleChecker* ac)
      : articleChecker(*ac), expectingMoreTasks(true)
  {
 ...
 private:  // functions
  ...
  bool inIsBlocked() const { return taskQueue.size() >= MAX_SIZE; }

  bool outIsBlocked() const { return expectingMoreTasks && taskQueue.empty(); }
  ...
  void unblockIn() { cv.notify_one(); }

  void unblockOut() { cv.notify_one(); }
...
{
 public:  // functions
  explicit TaskDispatcher(ArticleChecker* ac, unsigned n) : currentCluster(-1)
  {
    while (n--)
...
{
 private:  // types
  enum LoopStatus : uint8_t { UNKNOWN, LOOP, NONLOOP };
