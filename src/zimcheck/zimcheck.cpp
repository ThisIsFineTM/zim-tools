...
#include <zim-tools/zimcheck/checks.h>  // for EnabledTests, TestType, Error...
//
#include <zim/archive.h>  // for Archive
#include <zim/uuid.h>     // for operator<<
//
#include <docopt/docopt.h>        // for DocoptArgumentError, docopt_p...
#include <docopt/docopt_value.h>  // for value
//
#include <chrono>     // for duration, operator-, steady_c...
...
int zimcheck(const std::vector<const char*>& args)
{
  ...
  Options parsed_args;
  try {
    parsed_args = docopt::docopt_parse(USAGE, args_string, false, false);
...
int zimcheck(const Options& args)
{
  ...
  try {
    ...
    if (enabled_tests.isEnabled(TestType::INTEGRITY)) {
      ...
      error.infoMsg(
          "[WARNING] Integrity check is skipped. Any detected errors may in "
          "fact be due to corrupted/invalid data.");
    }
    ...
    if (should_run_full_test) {
       ...
       * }
       *
       * And so we will get several the data of an article (and so decompression
       * and so). By looping on the articles first, we have :
       *
       * for (article: articles) {
       *     data = article->getData();
       *     for (test: tests) {
